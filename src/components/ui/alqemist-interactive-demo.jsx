      ...
      <div className="rounded-xl border border-neutral-200 bg-neutral-100 p-1 shadow-2xl shadow-neutral-950/10 dark:border-neutral-800 dark:bg-neutral-900">
        ...
        <div className="mt-1 min-h-[430px] rounded-lg bg-white p-6 dark:bg-neutral-950">
          <div className="mb-6 flex items-start justify-between gap-4">
            <div>
              <h3 className="text-xl font-semibold text-[#0B2545] dark:text-white">{activeTab.label}</h3>
              <p className="text-sm text-neutral-500">A live-feeling product surface for enterprise workspace operations.</p>
            </div>
          ...
          <div className="grid gap-3 md:grid-cols-3">
            {['Connect tools', 'Run agent', 'Review output', 'Ship change', 'Audit access', 'Schedule follow-up'].map((item, index) => {
              const ItemIcon = index === 4 ? KeyRound : activeTab.icon;

              return (
                <div key={item} className="rounded-sm border border-neutral-200 p-4 dark:border-neutral-800 hover:border-[#0284C7] transition-colors">
                  <div className="mb-3 grid h-9 w-9 place-items-center rounded-sm bg-neutral-950 text-white dark:bg-white dark:text-neutral-950">
                    <ItemIcon className="w-4 h-4" />
                  </div>
                  <div className="font-medium text-[#0B2545] dark:text-white">{item}</div>
