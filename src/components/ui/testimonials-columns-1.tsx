          ...
          ...new Array(2).fill(0).map((_, index) => (
            <React.Fragment key={index}>
              {props.testimonials.map(({ text, image, name, role }, i) => (
                <div 
                  className="p-6 sm:p-7 rounded-2xl border border-[#D0E2F7] bg-white shadow-sm hover:shadow-luxury hover:border-[#0284C7] transition-all max-w-xs w-full space-y-4" 
                  key={i}
                >
                  ...
                  <div className="flex items-center gap-3 pt-3 border-t border-[#E2E8F0]">
                    <img
                      ...
                      src={image}
                      alt={name}
                      className="h-10 w-10 rounded-full object-cover border-2 border-[#0284C7]"
                      onError={(e) => {
                        e.currentTarget.src = "https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&w=200&q=80";
                      }}
                    />
                    <div className="flex flex-col">
                      <div className="font-heading font-bold text-xs sm:text-sm text-[#0B2545]">{name}</div>
